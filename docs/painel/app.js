...
import "./static/js/aiEntity.js?v=20260917j";
import { createTourModule } from "./static/js/tour.js?v=20260917j";
import { api, BASE, ErroApi } from "./api.js?v=20260917j";
...
function corpoVaga(config){
  ...
  return `<div class="vaga-monitor-card"><div class="vaga-monitor-media" role="img" aria-label="Carro elétrico conectado ao carregador">
    ...
    ${state.paineis.editando ? `<div class="vaga-monitor-config" data-vaga-config-box hidden><label>Vaga monitorada<select data-vaga-select aria-label="Escolher vaga monitorada">${carregadores.map(c => `<option value="${c.id}" ${c.id === escolhido?.id ? "selected" : ""}>${esc(c.nome || `Vaga #${c.id}`)}</option>`).join("")}</select></label></div>
